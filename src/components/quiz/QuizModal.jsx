...
export default function QuizModal({ quiz, onClose, onComplete }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[72] overflow-y-auto bg-black/35 p-3 backdrop-blur-sm sm:p-5"
      onClick={(event) => {
    ...
    >
      <div className="mx-auto flex h-full w-full max-w-4xl flex-col rounded-2xl border border-slate-200 bg-white shadow-2xl">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <span
              className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-semibold ${
                timeLeft <= 60 ? 'border-slate-400 bg-slate-100 text-slate-900' : 'border-slate-300 bg-slate-50 text-slate-700'
              }`}
            >
              <Timer size={12} />
              {formatTime(timeLeft)}
